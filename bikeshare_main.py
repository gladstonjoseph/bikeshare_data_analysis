...
def user_input_check(user_input, choose_list, category):
    ...
    while user_input not in choose_list:
        if category == "city":
            ...
            user_input = input(base_text + "a valid day of the week! : ").lower()  # NOQA
...
def get_filters():
    ...
    Returns:
        ...
        2. (str) month - name of the month to filter by, or "all" to apply no
        month filter
        3. (str) day - name of the day of week to filter by, or "all" to apply
        no day filter
    """

    print('Hello! Let\'s explore some US bikeshare data!\n')

    # get user input for city (chicago, new york city, washington). HINT: Use a while loop to handle invalid inputs  # NOQA
    user_city_input = input("Which city would you like to analyze ? \nChoose chicago, new york city, washington : ").lower()  # NOQA
    user_city_input = user_input_check(user_city_input, CITY_DATA, "city")
    city = CITY_DATA[user_city_input]

    # get user input for month (all, january, february, ... , june)
    user_month_input = input("\nWhich month would you like to analyze ? \nChoose any month from january to june. Choose 'all' if you want to see every month : ").lower()  # NOQA
    user_month_input = user_input_check(user_month_input,
                                        month_choose_list,
                                        "month")
    month = month_choose_list[user_month_input]

    # get user input for day of week (all, monday, tuesday, ... sunday)
    user_day_input = input("\nWhich day of the week would you like to analyze ? \nChoose any day of the week. Choose 'all' if you want to see all : ").lower()  # NOQA
    user_day_input = user_input_check(user_day_input,
                                      day_choose_list,
                                      "day_of_week")
...
def time_stats(df, city, month, day):
    ...
    if month == "all":
        ...
        for key, value in month_choose_list.items():
            if value == common_month[0]:
                print("The most common month is {}. A total of {} trips were made".format(key, common_month_count))  # NOQA
    ...
    if day == "all":
        ...
        for key, value in day_choose_list.items():
            if value == common_day[0]:
                print("The most common day of the week is {}. A total of {} trips were made".format(key, common_day_count))  # NOQA

    # display the most common start hour
    common_starthour = df["Start Time"].dt.hour.mode()
    common_starthour_count = df["Start Time"].dt.hour.value_counts().max()
    print("The most common starting hour of the trip is the {}th hour. A total of {} trips were made".format(common_starthour[0], common_starthour_count))  # NOQA
    ...
    print("The most common ending hour of the trip is the {}th hour. A total of {} trips were made".format(common_endhour[0], common_endhour_count))  # NOQA

    # display the average duration of all trips
    average_duration = df["Trip Duration"].mean() / 60
    print("The average duration of the trips is {} minutes".format(round(average_duration)))  # NOQA
...
def station_stats(df):
    ...
    print("The most common starting station is {}. A total of {} trips were made".format(common_startstation[0], common_startstation_count))  # NOQA

    # display most commonly used end station
    common_endstation = df["End Station"].mode()
    common_endstation_count = df["End Station"].value_counts().max()
    print("The most common ending station is {}. A total of {} trips were made".format(common_endstation[0], common_endstation_count))  # NOQA
    ...
    print("The most common trips start from {} and end at {}. A total of {} trips were made".format(common_journey[0], common_journey[1], common_journey_count))  # NOQA
...
def trip_duration_stats(df):
    ...
    print("The total travel time is {} hours".format(int(round(total_travel_time))))  # NOQA

    # display mean travel time
    average_travel_time = df["Trip Duration"].mean() / 60
    print("The average travel time per trip is {} minutes".format(int(round(average_travel_time))))  # NOQA
...
def user_stats(df):
    ...
    if "Gender" in df:
        ...
        for gender_index in gender_types:
            print("{}: {}".format(gender_index, gender_type_counts[gender_index]))  # NOQA
    ...
    if "Birth Year" in df:
        ...
        def print_user_stats(year, age, index, category):
            ...
            if "Gender" in df:
                ...
                print("The {} person to bike is {} years old. ".format(category, age) + gender_name + " was born in {}".format(year))  # NOQA
            else:
                print("The {} person to bike is {} years old. ".format(category, age) + gender_name + " was born in {}".format(year))  # NOQA
        ...
        print_user_stats(int(earliest_year),
                         oldest_age, earliest_year_index,
                         "oldest")
        ...
        print_user_stats(int(latest_year),
                         youngest_age,
                         latest_year_index,
                         "youngest")
        ...
        print("The most common age for biking is {}. The person was likely born in {}".format(most_common_age, int(most_common_year)))  # NOQA
...
def raw_trip_data(df, check):
    if check == 0:
        user_choice_raw_trip = input("\nWould you like to view individual trip data ? : ").lower()  # NOQA
        user_choice_raw_trip = user_input_check(user_choice_raw_trip,
                                                restart_choose_list,
                                                "yes_or_no")
    elif check > 0:
        user_choice_raw_trip = input("\nWould you like to see the next six rows ? : ").lower()  # NOQA
        user_choice_raw_trip = user_input_check(user_choice_raw_trip,
                                                restart_choose_list,
                                                "yes_or_no")
